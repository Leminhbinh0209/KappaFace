...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            ...
            results.append(acc2)
            report[self.ver_name_list[i]] = acc2
        return report
    ...
    def __call__(self, num_update, backbone: torch.nn.Module):
        if self.rank == 0 and num_update > 0 and num_update % self.frequent == 0:
            backbone.eval()
            r = self.ver_test(backbone, num_update)
            backbone.train()
            return r
...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone, partial_fc, to_save, state=None):
        
        if global_step > 100 and self.rank == 0:
            for dataset in to_save:
                path_module = os.path.join(self.output + f'/{dataset}', "backbone.pth")
                torch.save(backbone.module.state_dict(), path_module)
            if state is not None:
                path_state = os.path.join(self.output, "state.t7")
                torch.save(state, path_state)
